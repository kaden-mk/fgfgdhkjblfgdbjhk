	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'bopeebo' | 'fresh' | 'dad battle':
				FlxG.sound.playMusic(Paths.music('freshIntro'), 0);
				FlxG.sound.music.fadeIn(1, 0, 0.8);
		}
		...
		// Portraits for Player 2 and Player 1
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'bopeebo' | 'fresh' | 'dad battle':
				portraitLeft = new FlxSprite(-1500, 65);
				...
				portraitLeft.setGraphicSize(Std.int(portraitLeft.width * PlayState.daPixelZoom * 0.150));
		...
		if (FlxG.keys.justPressed.ANY  && dialogueStarted == true)
		{
			...
			if (dialogueList[1] == null && dialogueList[0] != null)
			{
				if (!isEnding)
				{
					...
					switch(PlayState.SONG.song.toLowerCase())
					{
						case 'senpai' | 'thorns' | 'bopeebo' | 'fresh' | 'dad battle':
	...
	function startDialogue():Void
	{
		...
		switch (curCharacter)
		{
			...
			case 'gfcheer':
				...
				{
					portraitLeft.visible = true;
					portraitLeft.frames = Paths.getSparrowAtlas('portraits/gfCheerPortrait');
					portraitLeft.animation.addByPrefix('enter', 'gfCheer Portrait', 24, false);
					portraitLeft.animation.play('enter');
				}
			case 'dad':
				portraitRight.visible = false;
